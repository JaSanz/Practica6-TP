...
public class ColorChooserExample extends JFrame {
	...
	boolean inicial1 = false;
	boolean inicial2 = false;
	private Color a = Color.BLUE;
	private Color z = Color.RED;
	...
	private void initGUI() {
		...
		JTable table = new JTable(tModel) {
			...
			public Component prepareRenderer(TableCellRenderer renderer, int row, int col) {
				...
				if (row == 0 && col == 1 && !inicial1) {
					comp.setBackground(a);
					inicial1 = true;
				}
				else if (row == 1 && col == 1 && !inicial2) {
					comp.setBackground(z);
					inicial2 = true;
				}
				else if (row == 0 && col == 1 && inicial1) {
					a = colors.get(row);
					comp.setBackground(a);
				}
				else if (row == 1 && col == 1 && inicial2) {
				...
				else if (row == 0 && col == 0) comp.setBackground(Color.WHITE);
				else if (row == 1 && col == 0) comp.setBackground(Color.WHITE);
				else comp.setBackground(colors.get(row));
